...
const CourseCardThin = ({ id, category, teacher, name, description, imageSrc, imageAlt, participants, positiveReviews, price }) => {
...
const CourseCardGrid = ({ courses }) => {
  return (
    <div className="flex justify-center">
      <div className="grid grid-cols-1 md:grid-cols-2  gap-6 px-0 sm:px-5">
        {courses.map((course, index) => (
          <CourseCardThin key={index} id={course._id} {...course} />
        ))}
      </div>
    </div>
  );
